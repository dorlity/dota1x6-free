...
function shop:UpdateFullParticleForPlayer(id, hero, for_this_id)
    ...
    if hero.other_model_backup_name ~= nil then
        for slot, name in pairs(hero.other_model_backup_name) do
            ...
            local item_info = shop:FindItemInfoOnModel(hero_name, item_model, hero:GetMaterialGroupHash())
...
-- Основная функция надевания предмета
function shop:SetItemToHero(hero, PlayerID, hero_name, item, starting)
    ...
    ------------- INIT TABLES ---------------------------------------------
    -- Сохраняем настоящую модель игрока
    ...
    -- Информация о всех надетых модельках 
    if hero.items_data_custom == nil then
		hero.items_data_custom = {}
	end

    -- Информация о всех надетых партиклах
    ...
    hero.items_slots_type[item_slot_type] = item -- Засейвить айди предмета в этом слоте

    -- Модификатор отвечающий за инвиз модельки, чтоб вераблы слетели
    ...
    --- Начинаем удалять предметы доты, похожие на предмет слота и если это аркана то удалять надо все предметы
    ...
    local callback = function()
        -- Добавление новой шмотки собственно !
        -- Кастомный предмет
...
-- Основная функция удаления предмета у героя
function shop:RemoveItemToHero(hero, PlayerID, hero_name, item_id)
    ...
    -- Если была изменена модель, то вернуть старую
...
function shop:BackupOtherModelDota(hero, item_slot_type)
    if hero.other_model_backup_name ~= nil and hero.other_model_backup_name[item_slot_type] ~= nil then
        ...
        local item_model_entity
        local item_info = shop:FindItemInfoOnModel(hero:GetUnitName(), item_model, hero:GetMaterialGroupHash())
...
function shop:FindItemInfoOnModel(hero_name, item_model, hash)
    if hero_name == "npc_dota_hero_juggernaut" and item_model == "models/items/juggernaut/arcana/juggernaut_arcana_mask.vmdl" then
        if tostring(hash) == "628863847" then
            for _, inf in pairs(shop.items_data[hero_name]) do
                if inf and inf["ItemModel"] == item_model and inf["MaterialGroup"] == "1" then
                    return inf
                end
            end
        else
            for _, inf in pairs(shop.items_data[hero_name]) do
                if inf and inf["ItemModel"] == item_model and inf["MaterialGroup"] == "0" then
                    return inf
                end
            end
        end
...
-- Прикрепить предметы с начала игры герою (ДОДЕЛАТЬ)
...
function shop:CopyTable(orig) -- Откопипастить таблицу
...
function shop.remove_item(tbl,item) -- Удалить итем
    local i,max=1,#tbl
    while i<=max do
        if tbl[i] == item then
            table.remove(tbl,i)
...
function shop:dota1x6_item_change(data) -- Вызов js ивента
	if data.PlayerID == nil then return end
...
-- Удаление дотовских предметов
function shop:RemoveAllItemsOtherModel(hero)
    -- Массив чтоб вернуть дефолтный предмет, если отключит кастомный с названиями
    if hero.other_model_backup_name == nil then
...
function shop:RemoveItemOtherModel(hero, model, item_id, player_id, hero_model, slot_type)
    ...
    if hero.other_model_backup ~= nil and hero.other_model_backup[slot_type] ~= nil then
        local item_info = shop:FindItemInfoOnModel(hero:GetUnitName(), hero.other_model_backup[slot_type]:GetModelName(), hero:GetMaterialGroupHash())
        if item_info then
            if hero:IsRealHero() then
                if hero.original_model_donate ~= "models/heroes/phantom_assassin_persona/phantom_assassin_persona.vmdl" then
...
---------- Сгенерировать информацию для CustomNetTable
