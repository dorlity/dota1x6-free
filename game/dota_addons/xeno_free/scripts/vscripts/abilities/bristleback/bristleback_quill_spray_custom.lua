...
if self:GetCaster():HasModifier("modifier_bristle_spray_heal") then 
  local heal = self:GetCaster():GetTalentValue("modifier_bristle_spray_heal", "heal")*(self:GetCaster():GetMaxHealth() - self:GetCaster():GetHealth())/100

  if is_passive then 
    heal = heal/self:GetCaster():GetTalentValue("modifier_bristle_spray_heal", "passive")
  end 
...
CreateModifierThinker(self.caster, self, "modifier_custom_bristleback_quillspray_thinker", {duration = self.duration, passive = passive,  cone = cone}, self.location, self.caster:GetTeamNumber(), false)
...
for _, enemy in pairs(enemies) do
  ...
  if angle_diff <= self.angle or self.cone == 0 then
    ...
    if not self.hit_enemies[enemy] then 
      ...
      if self.proc then 
        enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_custom_bristleback_quill_spray_proc_slow", {duration = self.slow_duration*(1 - enemy:GetStatusResistance())})
      end
