...
alchemist_unstable_concoction_throw_custom.weapon_stack = {2, 4}

alchemist_unstable_concoction_throw_custom.dispel = 2
 
alchemist_unstable_concoction_throw_custom.legendary_reduce = -20
alchemist_unstable_concoction_throw_custom.legendary_damage = 0.5
...
function alchemist_unstable_concoction_throw_custom:OnProjectileHit_ExtraData( target, location, ExtraData )
	...
	for _,enemy in pairs(enemies) do
		...
		local ability = self:GetCaster():FindAbilityByName("alchemist_corrosive_weaponry_custom")

		if self:GetCaster():HasModifier("modifier_alchemist_unstable_4") and ability then 
			for i = 1,self.weapon_stack[self:GetCaster():GetUpgradeStack("modifier_alchemist_unstable_4")] do 

				ability:AddStack(enemy)
			end
		end 
