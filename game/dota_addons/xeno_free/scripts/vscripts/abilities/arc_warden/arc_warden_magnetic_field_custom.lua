...
function arc_warden_magnetic_field_custom:GetBehavior()
  if self:GetCaster():HasModifier("modifier_arc_warden_field_7") then
    return DOTA_ABILITY_BEHAVIOR_AOE + DOTA_ABILITY_BEHAVIOR_IGNORE_BACKSWING + DOTA_ABILITY_BEHAVIOR_UNIT_TARGET
  end
...
if self:GetCaster():HasModifier("modifier_arc_warden_field_7") and self:GetCursorTarget() then 
	...
	if self:GetCaster():HasModifier("modifier_arc_warden_tempest_double") and not self:GetCaster():HasShard() then
		...
		self:GetCursorTarget():AddNewModifier(self:GetCaster(), self, "modifier_arc_warden_magnetic_field_custom_thinker_damage", { duration = duration } )
	else 

		name = "modifier_arc_warden_magnetic_field_custom_speed"
		self:GetCursorTarget():AddNewModifier(self:GetCaster(), self, "modifier_arc_warden_magnetic_field_custom_thinker_speed", { duration = duration } )
		self:GetCursorTarget():AddNewModifier(self:GetCaster(), self, "modifier_arc_warden_magnetic_field_custom_thinker_evasion", { duration = duration } )
	end 
