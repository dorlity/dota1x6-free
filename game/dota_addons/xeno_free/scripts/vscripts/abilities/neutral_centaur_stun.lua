...
function modifier_centaur_stun:OnIntervalThink()
	...
	self.sign = ParticleManager:CreateParticle("particles/generic_gameplay/generic_has_quest.vpcf", PATTACH_OVERHEAD_FOLLOW, parent)
		...
		for _, target in ipairs(targets) do
			if IsValidEntity(target) and not target:IsMagicImmune() then
				...
				if target:IsIllusion() then 
					damage = target:GetMaxHealth()*ability:GetSpecialValueFor("illusion")/100
				end
