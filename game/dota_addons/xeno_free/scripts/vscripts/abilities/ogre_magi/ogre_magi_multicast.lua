...
ogre_magi_multicast_custom.chance = {3,4,5}

ogre_magi_multicast_custom.fireball_radius = 1000
ogre_magi_multicast_custom.fireball_damage = 100
ogre_magi_multicast_custom.fireball_chance = {30, 45, 60}
...
ogre_magi_multicast_custom.proc_chance = 17
ogre_magi_multicast_custom.proc_bkb = 2.5
ogre_magi_multicast_custom.proc_bkb_magic = 50 
ogre_magi_multicast_custom.proc_silence = 2.5
ogre_magi_multicast_custom.proc_damage = 10
...
for i = 0, 5 do
    ...
    if current_item and not NoCdItems[current_item:GetName()] and current_item:GetCooldownTimeRemaining() > 0 then  
        items[#items + 1] = i
    end
end
...
function ogre_magi_multicast_custom:GetChannelTime()
	return self:GetCaster():GetTalentValue("modifier_ogremagi_multi_7", "duration")
end
...
if not IsServer() then return end
local chance = self.fireball_chance[self:GetCaster():GetUpgradeStack("modifier_ogremagi_multi_3")]
...
function modifier_ogre_magi_multicast_custom:OnAbilityFullyCast( params )
	...
	local luck = self:GetParent():FindAbilityByName("ogre_magi_dumb_luck_custom")

	if self:GetParent():HasModifier("modifier_ogremagi_multi_1") then 
		if chance_2 > 0 then 
			chance_2 = chance_2 + self:GetAbility().chance[self:GetParent():GetUpgradeStack("modifier_ogremagi_multi_1")]
		end

		if chance_3 > 0 then 
			chance_3 = chance_3 + self:GetAbility().chance[self:GetParent():GetUpgradeStack("modifier_ogremagi_multi_1")]
		end

		if chance_4 > 0 then 
			chance_4 = chance_4 + self:GetAbility().chance[self:GetParent():GetUpgradeStack("modifier_ogremagi_multi_1")]
		end
	end
	...
	if self:GetParent():GetQuest() == "Ogre.Quest_8" and target and target:IsBaseNPC() and target:IsRealHero() and target:GetTeamNumber() ~= self:GetCaster():GetTeamNumber() then 
	...
	if self:GetCaster():HasModifier("modifier_ogremagi_multi_4") then 
		self:GetParent():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_ogre_magi_multicast_custom_attack", {duration = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_4", "duration")})
	end
	...
	or params.ability:GetName() == "ogre_magi_bloodlust_custom" or params.ability:GetName() == "ogre_magi_smash_custom")  then 
		...
		if self:GetCaster():HasModifier("modifier_ogremagi_multi_7") then
			self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_ogre_magi_multicast_custom_legendary_spell", {duration = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_7", "spells"), name = params.ability:GetName()})
		end
...
function modifier_ogre_magi_multicast_custom_use:OnIntervalThink()
	...
	or self.ability:GetName() == "ogre_magi_bloodlust_custom" or self.ability:GetName() == "ogre_magi_smash_custom")  then 
		...
		if self:GetCaster():HasModifier("modifier_ogremagi_multi_7") then 
			self:GetCaster():AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_ogre_magi_multicast_custom_legendary_spell", {duration = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_7", "spells"), name = self.ability:GetName()})
		end
...
self.radius = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_7", "radius")
...
local interval = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_7", "duration")/#self.skills - 0.05
...
local units = {}
local creeps = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_BASIC, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_ANY_ORDER, false )
local heroes = FindUnitsInRadius( self:GetParent():GetTeamNumber(), self:GetParent():GetOrigin(), nil, self.radius, DOTA_UNIT_TARGET_TEAM_ENEMY, DOTA_UNIT_TARGET_HERO, DOTA_UNIT_TARGET_FLAG_FOW_VISIBLE, FIND_ANY_ORDER, false )
...
if fireball then 
	...
	if self:GetParent():HasModifier("modifier_ogremagi_multi_3") then 
		self:GetAbility():Fireball()
	end
...
self.max = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_4", "max")
self.range = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_4", "range")
self.damage = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_4", "damage")
self.radius = self:GetCaster():GetTalentValue("modifier_ogremagi_multi_4", "radius")
...
function modifier_ogre_magi_multicast_custom_attack:GetModifierAttackRangeBonus()
if self:GetStackCount() < self.max then return end
return self.range
end
...
self:GetParent():GenericHeal(damage, self:GetAbility())
...
function modifier_ogre_magi_multicast_custom_attack:OnStackCountChanged(iStackCount)
if not IsServer() then return end
...
if self:GetStackCount() == self.max then 
	...
	self.effect_cast_2 = ParticleManager:CreateParticle( "particles/ogre_head.vpcf", PATTACH_OVERHEAD_FOLLOW, self:GetParent() )
