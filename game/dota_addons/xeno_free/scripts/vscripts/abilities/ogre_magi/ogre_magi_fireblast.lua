...
ogre_magi_fireblast_custom.damage_bonus = {30,45,60}

ogre_magi_fireblast_custom.heal = {0.1, 0.15, 0.2}
...
ogre_magi_fireblast_custom.legendary_range = 800
...
ogre_magi_fireblast_custom.fire_damage = {15, 25}
ogre_magi_fireblast_custom.fire_chance = 100
ogre_magi_fireblast_custom.fire_heal_creeps = 0.33
ogre_magi_fireblast_custom.fire_heal = 1
ogre_magi_fireblast_custom.fire_max = 6
...
ogre_magi_fireblast_custom.fullhp_cast = 0.15

ogre_magi_unrefined_fireblast_custom = class({})
ogre_magi_unrefined_fireblast_custom.legendary_range = 800
ogre_magi_unrefined_fireblast_custom.legendary_creep = 0.5

ogre_magi_unrefined_fireblast_custom.fullhp_cast = 0.15
...
if self:GetCaster():HasModifier('modifier_ogremagi_blast_5') then 
    return self.BaseClass.GetCastPoint(self) - self.fullhp_cast
end
...
function ogre_magi_fireblast_custom:Impact(target, dont_stun)
...
if self:GetCaster():HasModifier("modifier_ogremagi_multi_5") then 
	...
	if RollPseudoRandomPercentage(ability.proc_chance, 527, self:GetCaster()) then
...
self.cleave = self:GetSpecialValueFor( "cleave_damage" )/100
...
if not self:GetCaster():HasModifier("modifier_ogremagi_blast_7") then 
 
	local aoe_targets = self:GetCaster():FindTargets(self:GetSpecialValueFor("cleave_radius"), target:GetAbsOrigin())

	for _,aoe_target in pairs(aoe_targets) do 
		if aoe_target ~= target then
			ApplyDamage( { victim = aoe_target, attacker = self:GetCaster(), damage = self.damage*self.cleave, damage_type = self:GetAbilityDamageType(), ability = self } ) 
		end 
	end 
end 
...
function ogre_magi_unrefined_fireblast_custom:Impact(target, dont_stun)
if not IsServer() then return end
...
self.damage = self:GetSpecialValueFor( "fireblast_damage" ) + self:GetCaster():GetStrength()*self:GetSpecialValueFor("str_damage")/100
...
if not target then 
	...
	local damage = self:GetSpecialValueFor( "fireblast_damage" ) + self:GetCaster():GetStrength()*self:GetSpecialValueFor("str_damage")/100
...
if self:GetCaster() == params.attacker and self:GetCaster():HasModifier("modifier_ogremagi_blast_2") and params.inflictor 
	and (params.inflictor:GetName() == "ogre_magi_fireblast_custom" or params.inflictor:GetName() == "ogre_magi_unrefined_fireblast_custom") then
...
if not self:GetParent():HasModifier("modifier_ogremagi_blast_6") then return end
if self:GetParent() ~= params.unit then return end
if self:GetParent():PassivesDisabled() then return end
if self:GetParent():HasModifier("modifier_ogre_magi_fireblast_lowhp_cd") then return end
...
for _,enemy in pairs(enemies) do
	self:GetAbility():Impact(enemy, false)
...
local damage = ApplyDamage( { victim = self:GetParent(), attacker = self:GetCaster(), damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility() } )

local k = self:GetAbility().fire_heal
if not self:GetParent():IsHero() then 
	k = self:GetAbility().fire_heal_creeps
end
