...
function modifier_item_echo_spear_custom_enemy:OnCreated(params)
    ...
    ApplyDamage({attacker = self:GetCaster(), victim = self:GetParent(), damage = self.damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility()})
...
function modifier_item_echo_spear_custom_enemy:CheckState()
return
{
