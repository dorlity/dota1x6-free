...
function item_observer_stackable:OnAbilityPhaseStart()
    ...
    if  self:GetCurrentCharges() <= 0 then
        CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetCaster():GetPlayerOwnerID()), "CreateIngameErrorMessage", {message = "#dota_hud_error_no_charges"})
        return false
    end
    return true
end
