...
lina_light_strike_array_custom.shard_damage = 20
...
lina_light_strike_array_custom.stun_inc = 0.4
...
lina_light_strike_array_custom.double_heal = 1
...
PrecacheResource( "particle", "particles/lina_array_triple.vpcf", context )
PrecacheResource( "particle", "particles/lina_timer.vpcf", context )
PrecacheResource( "particle", "particles/lina_second_blast.vpcf", context )
...
function modifier_lina_light_strike_array_custom:OnCreated( kv )
  ...
  if self:GetCaster():HasModifier("modifier_lina_array_5") then 
    self.stun_duration = self.stun_duration + self:GetAbility().stun_inc
  end
...
for _,enemy in pairs(enemies) do
  ...
  if self:GetCaster():HasModifier("modifier_lina_dragon_1") then 
    enemy:AddNewModifier(self:GetCaster(), self:GetAbility(), "modifier_lina_dragon_slave_custom_magic", {duration = self:GetCaster():GetTalentValue("modifier_lina_dragon_1", "duration")})
  end 
...
function modifier_lina_light_strike_array_custom_after:IsHidden() return false end
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'lina_array_change',  {hide = 0, max_time = self.time, time = self:GetRemainingTime(), damage = self.damage})
...
local damage = params.damage*(self:GetAbility().double_init + self:GetAbility().double_inc*self:GetCaster():GetUpgradeStack("modifier_lina_array_4"))
...
CustomGameEventManager:Send_ServerToPlayer(PlayerResource:GetPlayer(self:GetParent():GetPlayerOwnerID()), 'lina_array_change',  {hide = 1, max_time = self.time, time = self:GetRemainingTime(), damage = self.damage})


if not self:GetParent():IsAlive() then return end


self:GetParent():GenericHeal(self.damage * self:GetAbility().double_heal, self:GetAbility())
