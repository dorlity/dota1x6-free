...
hoodwink_bushwhack_custom.incoming_heal = {-4, -6}
...
function modifier_hoodwink_bushwhack_custom_thinker:OnDestroy()
	...
	if self:GetCaster():HasModifier("modifier_hoodwink_bush_legendary") then
		...
		if #trees>0 and #enemies<1 then
			...
			self:GetAbility():StartCooldown(cd*self:GetCaster():GetTalentValue("modifier_hoodwink_bush_legendary", "cd")/100)

			local mana = self:GetAbility().BaseClass.GetManaCost(self:GetAbility(),self:GetAbility():GetLevel())*self:GetCaster():GetTalentValue("modifier_hoodwink_bush_legendary", "mana")/100
...
return self:GetStackCount()*(self:GetAbility().incoming_heal[self:GetCaster():GetUpgradeStack("modifier_hoodwink_bush_4")])
end
