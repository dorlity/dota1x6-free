...
skills = {
	all = {
		...
		{"modifier_up_allstats",0, "gray" ,0,"gray_item","circlet",25,2}, 
		...
		{"modifier_up_gainall",0,"blue",3,"blue_item","Crown",22,0},
		...
		{"modifier_up_allupgrade",0,"purple",1,"purple_item","Apex",19,0},
	...
	npc_dota_hero_sand_king = {
		...
		{"modifier_sand_king_finale_2",1,"blue",3,"blue_skill","finale",25,3,"finale_2",0},
		...
		{"modifier_sand_king_burrow_5",1,"purple",1,"purple_skill","burrow",25,1,"burrow_5",1},
		{"modifier_sand_king_burrow_6",1,"purple",1,"purple_skill","burrow",25,1,"burrow_6",1},

		{"modifier_sand_king_sand_4",1,"purple",2,"purple_skill","sand",25,2,"sand_4",0},
		{"modifier_sand_king_sand_5",1,"purple",1,"purple_skill","sand",25,2,"sand_5",1},
		{"modifier_sand_king_sand_6",1,"purple",1,"purple_skill","sand",25,2,"sand_6",1},

		{"modifier_sand_king_finale_4",1,"purple",2,"purple_skill","finale",25,3,"finale_4",1},
		{"modifier_sand_king_finale_5",1,"purple",1,"purple_skill","finale",25,3,"finale_5",0},
		{"modifier_sand_king_finale_6",1,"purple",1,"purple_skill","finale",25,3,"finale_6",1},

		{"modifier_sand_king_epicenter_4",1,"purple",2,"purple_skill","epicenter",25,4,"epicenter_4",0},
		{"modifier_sand_king_epicenter_5",1,"purple",1,"purple_skill","epicenter",25,4,"epicenter_5",1},
		{"modifier_sand_king_epicenter_6",1,"purple",1,"purple_skill","epicenter",25,4,"epicenter_6",1},
...
_G.perma_upgrade =
{
	["modifier_sniper_assassinate_6"] = {mod = "modifier_sniper_assassinate_custom_kill_stack", max = 8},
	["modifier_phantom_assassin_crit_steal"] = {mod = "modifier_phantom_assassin_phantom_coup_de_grace_kill", max = 8},
	["modifier_nevermore_souls_kills"] = {mod = "modifier_custom_necromastery_kills", max = 10},
	["modifier_puck_coil_magic"] = {mod = "modifier_custom_puck_dream_coil_kills", max = 12},
	["modifier_axe_culling_6"] = {mod = "modifier_axe_culling_blade_custom_kills", max = 8},
	["modifier_alchemist_greed_6"] = {mod = "modifier_alchemist_goblins_greed_custom_runes", max = 35},
	["modifier_templar_assassin_meld_6"] = {mod = "modifier_templar_assassin_meld_custom_kills", max = 8},
	["modifier_zuus_wrath_6"] = {mod = "modifier_zuus_thundergods_wrath_custom_kills", max = 8},
	["modifier_mars_spear_4"] = {mod = "modifier_mars_spear_custom_str", max = 20},
	["modifier_primal_beast_onslaught_4"] = {mod = "modifier_primal_beast_onslaught_custom_stacks", max = 15},
	["modifier_pudge_hook_4"] = {mod = "modifier_custom_pudge_meat_hook_legendary", max = 20},
	["modifier_troll_axes_4"] = {mod = "modifier_troll_warlord_whirling_axes_perma_melee", max = 12},
	["modifier_skeleton_vampiric_4"] = {mod = "modifier_skeleton_king_vampiric_aura_stats",},
	["modifier_skeleton_reincarnation_6"] = {mod = "modifier_skeleton_king_reincarnation_custom_str", max = 8},
	["modifier_razor_link_4"] = {mod = "modifier_razor_static_link_custom_perma", max = 12},
	["modifier_legion_duel_blood"] = {mod = "modifier_duel_custom_str", max = 80},
...
function upgrade:CheckSkill(player, number, rare)
	local rarity = upgrade:GetRarityName(rare)
...
function upgrade:RandomCheck(i, array)
	local f = true
...
function upgrade:SortSkill(rare, player, after_legen)
	local sort = {}
	local j = 0
	local f = false
	local start = 1
	...
	if players[player:GetTeamNumber()].chosen_skill ~= 0 and (after_legen == true or test == true) and
			(upgrade:RandomCheck( players[player:GetTeamNumber()].chosen_skill, players[player:GetTeamNumber()].ban_skills ))
	 then
	...
	for i = start, 2 do
		repeat
			...
			if not upgrade:RandomCheck(j, players[player:GetTeamNumber()].ban_skills) then
				f = false
			end
		until f == true
...
function check_type(player, types, rarity, name)
	...
	local only_normal =
	{
		["modifier_up_primary"] = true,
		["modifier_up_secondary"] = true,
		["modifier_up_gainprimary"] = true,
		["modifier_up_gainsecondary"] = true,
		["modifier_up_secondaryupgrade"] = true,
		["modifier_up_primaryupgrade"] = true,
	}
	local only_all =
	{
		["modifier_up_allstats"] = true,
		["modifier_up_allupgrade"] = true,
		["modifier_up_gainall"] = true
	}



	if only_normal[name] and players[player:GetTeamNumber()]:GetPrimaryAttribute() == DOTA_ATTRIBUTE_ALL then
		return false
	end

	if only_all[name] and players[player:GetTeamNumber()]:GetPrimaryAttribute() ~= DOTA_ATTRIBUTE_ALL then
		return false
	end
	...
	for _, group_name in pairs({player:GetUnitName(), "all"}) do
		for _, data in pairs(skills[group_name]) do
			...
			if not banned then
				...
				if  (rare == 1 or data[8] == skill or skill == 5)
					and check_type(player, data[3], rarity, data[1])
...
function upgrade:init_upgrade(player, rarity, can_refresh, after_legen, prev_choise, instant)
	...
	if rarity ~= 10 and rarity ~= 11 and rarity ~= 12 and rarity ~= 13 then
		...
		elseif rarity == 1 then
			...
			for i = 1, 4 do
				if not upgrade:CheckSkill(player, i, rarity) then
					j = j + 1
			...
			if j == 4 then
				if rarity == 2 then
					...
					for i = 1, 4 do
						if not upgrade:CheckSkill(player, i, rarity - 1) then
							t = t + 1
					...
					if t >= 4 then
						...
						local skill_number = upgrade:SortSkill(rarity - 1, player, after_legen)
				...
				local skill_number = upgrade:SortSkill(rarity, player, after_legen)
	...
	for i = 1, #players[player:GetTeamNumber()].choise do
		...
		if perma_upgrade[players[player:GetTeamNumber()].choise[i]] then 
			local mod = player:FindModifierByName(perma_upgrade[players[player:GetTeamNumber()].choise[i]].mod)

			perma_info[i].stack = 0
			perma_info[i].max = -1 

			if perma_upgrade[players[player:GetTeamNumber()].choise[i]].max then 
				perma_info[i].max = perma_upgrade[players[player:GetTeamNumber()].choise[i]].max
			end 
