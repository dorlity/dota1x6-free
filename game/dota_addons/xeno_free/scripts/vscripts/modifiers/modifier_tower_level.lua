...
self:GetParent():SetBaseMaxHealth(self:GetParent():GetBaseMaxHealth()*1.050)
...
function modifier_tower_level:GetModifierTotalDamageOutgoing_Percentage( params ) 
  ...
  if params.target:IsHero() and not params.inflictor then 
	-- return -50
	end 
end
end
...
local sound = ''
  if params.unit:IsBuilding() and params.unit:GetTeamNumber() == self:GetParent():GetTeamNumber() then
    sound = "Tower.Attack"--"Base.Attack"
