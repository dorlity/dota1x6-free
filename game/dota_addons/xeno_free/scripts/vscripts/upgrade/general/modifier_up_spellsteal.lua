...
if bit.band(param.damage_flags, DOTA_DAMAGE_FLAG_REFLECTION) == DOTA_DAMAGE_FLAG_REFLECTION then return end
    if self:GetParent() == param.attacker and param.inflictor and not param.unit:IsBuilding() and not param.unit:IsIllusion() then 
      ...
      self:GetParent():Heal(param.damage * (heal*self:GetStackCount()*(1 + k +0.3*self:GetParent():GetUpgradeStack("modifier_up_graypoints")) / 100), self:GetParent())
