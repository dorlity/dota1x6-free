modifier_razor_arcana_v2_custom = class({})
function modifier_razor_arcana_v2_custom:IsHidden() return true end
function modifier_razor_arcana_v2_custom:IsPurgable() return false end
function modifier_razor_arcana_v2_custom:IsPurgeException() return false end
function modifier_razor_arcana_v2_custom:RemoveOnDeath() return false end
function modifier_razor_arcana_v2_custom:DeclareFunctions()
    return
    {
...
function modifier_razor_arcana_v2_custom:OnDestroy()
...
function modifier_razor_arcana_v2_custom:OnAttackLanded(params)
    ...
    if length >= 200 then
        local particle_on_hit = ParticleManager:CreateParticle("particles/econ/items/razor/razor_arcana/razor_arcana_v2_base_attack.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_razor_arcana_v2_custom:OnDeath(params)
    if not IsServer() then return end
    if params.attacker == self:GetParent() and params.unit:IsRealHero() and params.unit ~= self:GetParent() then
        -- Death kill effect
        local caster_effect = ParticleManager:CreateParticle("particles/econ/items/razor/razor_arcana/razor_arcana_kill_effect_caster_v2.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
	...
	local nFXIndex = ParticleManager:CreateParticle("particles/econ/items/razor/razor_arcana/razor_arcana_death_v2.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent())
...
function modifier_razor_arcana_v2_custom:GetAttackSound()
