...
function skeleton_king_mortal_strike_custom:GetAbilityTextureName()
    if self:GetCaster():HasModifier("modifier_wraith_king_ti_8_weapon") then
        return "skeleton_king/ti8_immortal_weapon/skeleton_king_mortal_strike_immortal"
    end
    if self:GetCaster():HasModifier("modifier_wraith_king_ti_8_weapon_crimson") then
        return "skeleton_king/ti8_immortal_weapon/skeleton_king_mortal_strike_immortal_crimson"
    end
    if self:GetCaster():HasModifier("modifier_wraith_king_arcana_custom_v1") then
        return "skeleton_king/arcana/skeleton_king_mortal_strike_arcana"
    end
    if self:GetCaster():HasModifier("modifier_wraith_king_arcana_custom_v2") then
        return "skeleton_king/arcana/skeleton_king_mortal_strike_arcana_alt"
    end
...
if self.record and self.record == params.record then
    ...
    local sound = "Hero_SkeletonKing.CriticalStrike"
    if self:GetCaster():HasModifier("modifier_wraith_king_ti_8_weapon") or self:GetCaster():HasModifier("modifier_wraith_king_ti_8_weapon_crimson") then
        sound = "Hero_SkeletonKing.CriticalStrike.TI8"
    end

    self:GetParent():EmitSound(sound)
 	

    local part = "particles/wraith_king/crit_normal.vpcf"

    if self:GetCaster():HasModifier("modifier_wraith_king_arcana_custom_v1") then
        part = "particles/wraith_king/crit_arcana_v1.vpcf"
    end
    if self:GetCaster():HasModifier("modifier_wraith_king_arcana_custom_v2") then
        part = "particles/wraith_king/crit_arcana_v2.vpcf"
    end

    if self:GetCaster():HasModifier("modifier_wraith_king_winterblight_custom") then 

        local effect_cast = ParticleManager:CreateParticle( "particles/econ/items/wraith_king/wraith_king_winterblight_weapon/wraith_king_winterblight_blur_critical.vpcf", PATTACH_ABSORIGIN_FOLLOW, self:GetParent() )
