...
if self:GetParent():IsAlive() then 
    ...
    local damage = self:GetParent():GetStrength()*self:GetAbility():GetSpecialValueFor("pct_str_damage_per_second")*0.5/100

    damage = math.min(damage, self:GetAbility():GetSpecialValueFor("max_damage")*0.5)
    ...
    for _,enemy in pairs(enemies) do
        local damageTable = { victim = enemy, attacker = self:GetParent(), damage = damage, damage_type = DAMAGE_TYPE_MAGICAL, ability = self:GetAbility(), damage_flags = DOTA_DAMAGE_FLAG_NONE, }
