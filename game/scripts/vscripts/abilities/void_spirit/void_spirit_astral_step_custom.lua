...
LinkLuaModifier("modifier_void_spirit_astral_step_resist", "abilities/void_spirit/void_spirit_astral_step_custom", LUA_MODIFIER_MOTION_NONE)
...
if self:GetCaster():HasModifier("modifier_void_step_1") then 
	self.damage = self.damage + self:GetCaster():GetTalentValue("modifier_void_step_1", "damage")
end
...
self.resist_duration = self:GetCaster():GetTalentValue("modifier_void_step_1", "duration", true)
end 
...
params.target:AddNewModifier(self:GetParent(), self:GetAbility(), "modifier_void_spirit_astral_step_resist", {duration = self.resist_duration})
end 
...
modifier_void_spirit_astral_step_resist = class({})
function modifier_void_spirit_astral_step_resist:IsHidden() return false end
function modifier_void_spirit_astral_step_resist:IsPurgable() return true end
function modifier_void_spirit_astral_step_resist:GetTexture() return "buffs/remnant_lowhp" end
function modifier_void_spirit_astral_step_resist:OnCreated()

self.max = self:GetCaster():GetTalentValue("modifier_void_step_1", "max")
self.magic = self:GetCaster():GetTalentValue("modifier_void_step_1", "resist")
if not IsServer() then return end 
...
function modifier_void_spirit_astral_step_resist:OnRefresh()
if not IsServer() then return end
if self:GetStackCount() >= self.max then return end 
...
function modifier_void_spirit_astral_step_resist:DeclareFunctions()
return
{
...
function modifier_void_spirit_astral_step_resist:GetModifierMagicalResistanceBonus()
